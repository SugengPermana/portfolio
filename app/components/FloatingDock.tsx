    ...
    >
      <AnimatePresence>
        {open && (
          <motion.div
            layoutId="nav"
            className="absolute right-1 bottom-full mb-2 flex flex-col gap-2"
          >
            {items.map((item, idx) => (
              <motion.div
                key={item.title}
...
const FloatingDockDesktop = ({
  ...
  return (
    <motion.div
      ...
      className={cn(
        // bg-gray-100 (white) dark:bg-gray-900 (black)
        "border dark:border-gray-400 fixed bottom-6 left-1/2 -translate-x-1/2 z-50 hidden min-[580px]:flex h-16 items-end gap-4 rounded-2xl bg-gray-100 px-4 pb-3 dark:bg-neutral-900 shadow-xl",
        className,
      )}
    >
      {items.map((item) => (
...
function DockThemeToggle({ mouseX }: { mouseX: MotionValue }) {
  ...
  return (
    <motion.div
      ...
      className="relative flex aspect-square items-center justify-center rounded-full bg-gray-200 dark:bg-neutral-800 overflow-hidden"
    >
      <AnimatePresence>
        {hovered && (
          <motion.div
